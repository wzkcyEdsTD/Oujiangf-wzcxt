<template>
  <div class="index">
    <div class="mes" v-for="(value, index) of jobList" :key="index">
      <li class="text" @click="back(value,marriage,marriagename,political,address)">{{ value }}</li>
    </div>
  </div>
</template>
<script>
    import jobListData from './job.json';
    export default {
        name: "job",
        data() {
            const jobList = jobListData.sort(function (item, nextItem) {
              const number1 = parseInt(Object.keys(item)[0]),
                number2 = parseInt(Object.keys(nextItem)[0]);
